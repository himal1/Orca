...
NSString* ORRaspberryPiAuthTokenChanged       = @"ORRaspberryPiAuthTokenChanged";
NSString* ORRaspberryPiOrgChanged             = @"ORRaspberryPiOrgChanged";
NSString* ORRaspberryPiBucketChanged          = @"ORRaspberryPiBucketChanged";
NSString* ORRaspberryPiHostNameChanged        = @"ORRaspberryPiHostNameChanged";
NSString* ORRaspberryPiModelDBInfoChanged     = @"ORRaspberryPiModelDBInfoChanged";
NSString* ORRaspberryPiRateChanged            = @"ORRaspberryPiRateChanged";
NSString* ORRaspberryPiStealthModeChanged     = @"ORRaspberryPiStealthModeChanged";
NSString* ORRaspberryPiBucketArrayChanged     = @"ORRaspberryPiBucketArrayChanged";
NSString* ORRaspberryPiOrgArrayChanged        = @"ORRaspberryPiOrgArrayChanged";
NSString* ORRaspberryPiErrorChanged           = @"ORRaspberryPiErrorChanged";
NSString* ORRaspberryPiConnectionStatusChanged= @"ORRaspberryPiConnectionStatusChanged";
NSString* ORRaspberryPiMaxLineCountChanged    = @"ORRaspberryPiMaxLineCountChanged";
NSString* ORRaspberryPiMeasurementTimeOutChanged    = @"ORRaspberryPiMeasurementTimeOutChanged";
NSString* ORRaspberryPiLock                   = @"ORRaspberryPiLock";

static NSString* ORRaspberryPiModelInConnector = @"ORRaspberryPiModelInConnector";

@interface ORRaspberryPiModel (private)
...
- (void) setUpImage
{
    [self setImage:[NSImage imageNamed:@"RaspberryPi"]];
}
...
- (void) dataFileNameChanged:(NSNotification*)aNote
{
    ...
    ORRaspberryPiMeasurement* aCmd = [ORRaspberryPiMeasurement measurementForBucket:@"ORCA" org:org];
    [aCmd start   : @"DataFile"];
...
- (void) addRemoteMeasurement:(NSNotification*)aNote
{
    ...
    if(record){
        //get bucket
        ORRaspberryPiMeasurement* aCmd = [ORRaspberryPiMeasurement measurementForBucket:[record objectForKey:@"bucket"] org:org];
...
{
    return [ORRaspberryPiMeasurement measurementForBucket:aBucket org:org];
}
...
- (void) setConnectionStatus:(ORRaspberryPiConnectionStatus)status
{
    switch(status){
        ...
        case kInFluxDBConnectionOK:
            ...
            [connectionAlarm release];
            connectionAlarm = nil;
            [[NSNotificationCenter defaultCenter] postNotificationName:ORRaspberryPiConnectionStatusChanged object:self];
            break;
...
- (id) nextObject
{
    return [self objectConnectedTo:ORRaspberryPiModelInConnector];
}
...
- (void) setHostName:(NSString*)aHostName
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORRaspberryPiHostNameChanged object:self];
}
...
- (void) setAuthToken:(NSString*)aAuthToken
{
    ...
    [self setConnectionStatusUnknown];
    [[NSNotificationCenter defaultCenter] postNotificationName:ORRaspberryPiAuthTokenChanged object:self];
}
...
- (void) setOrg:(NSString*)anOrg
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORRaspberryPiOrgChanged object:self];
}
...
- (void) setStealthMode:(BOOL)aStealthMode
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ORRaspberryPiStealthModeChanged object:self];
}
...
- (void)  setMeasurementTimeOut:(short)aValue
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationOnMainThreadWithName:ORRaspberryPiMeasurementTimeOutChanged object:self];
}
...
- (void)  setMaxLineCount:(short) aValue
{
    ...
    maxLineCount = aValue;
    [[NSNotificationCenter defaultCenter] postNotificationOnMainThreadWithName:ORRaspberryPiMaxLineCountChanged object:self];
}
- (void) startTimer
{
    [timer invalidate];
...
- (void) calcRate
{
    ...
    totalSent = 0;
    [[NSNotificationCenter defaultCenter] postNotificationOnMainThreadWithName:ORRaspberryPiRateChanged object:self];
}
...
- (void) bufferMeasurement:(ORRaspberryPiMeasurement*)aCmd
{
...
- (void) consolidateAndSendCmdArray:(NSMutableArray*)cmds
{
    ...
    ORRaspberryPiMeasurement* firstCmd = [cmds firstObject]; //need the bucket and org. will be same for all here
    ORRaspberryPiCmdLineMode* aCombinedCmd = [ORRaspberryPiCmdLineMode lineModeForBucket:[firstCmd bucket] org:[firstCmd org]];
    for(ORRaspberryPiMeasurement* aCmd in cmds){
...
//        }
    ...
    ORRaspberryPiMeasurement* aCmd = [ORRaspberryPiMeasurement measurementForBucket:@"Logs" org:org];
...
- (void) setErrorString:(NSString*)anError
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationOnMainThreadWithName:ORRaspberryPiErrorChanged
                                    object:self
                                  userInfo:nil
                             waitUntilDone:NO];
...
- (void) updateAlarmState:(ORAlarm*)anAlarm
{
    ...
    ORRaspberryPiMeasurement* aCmd = [ORRaspberryPiMeasurement measurementForBucket:@"Alarms" org:org];
    [aCmd start:    @"Alarm"];
...
- (void) deleteBucket:(NSInteger)index
{
    ...
    if(aBucketId){
        ORRaspberryPiDeleteBucket* aCmd = [ORRaspberryPiDeleteBucket deleteBucket];
        [aCmd setBucketId:[bucketInfo objectForKey:@"id"]];
        [self executeDBCmd:aCmd];
        [self performSelector:@selector(executeDBCmd:) withObject:[ORRaspberryPiListBuckets listBuckets] afterDelay:2];
...
{
    if(experimentName){
        [self executeDBCmd:[ORRaspberryPiCreateBucket createBucket:experimentName
                                                          orgId:[self orgId] expireTime:60*60*24*7]];
    }
    [self executeDBCmd:[ORRaspberryPiCreateBucket createBucket:@"Logs"
                                                      orgId:[self orgId] expireTime:60*60*24*60]];
    
    [self executeDBCmd:[ORRaspberryPiCreateBucket createBucket:@"HWMaps"
                                                      orgId:[self orgId] expireTime:60*60*24*60]];

    [self executeDBCmd:[ORRaspberryPiCreateBucket createBucket:@"ORCA"
                                                      orgId:[self orgId] expireTime:60*60*24*14]];
    
    [self executeDBCmd:[ORRaspberryPiCreateBucket createBucket:@"Sensors"
                                                      orgId:[self orgId] expireTime:60*60*24*60]];
    
    [self executeDBCmd:[ORRaspberryPiCreateBucket createBucket:@"Computer"
                                                      orgId:[self orgId] expireTime:60*60*24*10]];
    
    [self executeDBCmd:[ORRaspberryPiCreateBucket createBucket:@"Alarms"
                                                      orgId:[self orgId] expireTime:60*60*24*10]];

    [self executeDBCmd:[ORRaspberryPiCreateBucket createBucket:@"SlowControls"
                                                      orgId:[self orgId] expireTime:60*60*24*10]];

    
    [self performSelector:@selector(executeDBCmd:) withObject:[ORRaspberryPiListBuckets listBuckets] afterDelay:1];
}
...
- (void) updateOrcaResources
{
    ...
    ORRaspberryPiMeasurement* aMeasurement;
    aMeasurement  = [ORRaspberryPiMeasurement measurementForBucket:@"ORCA" org:org];
...
- (void) updateDiskInfo
{
    ...
    for(id aVolume in diskInfo){
        ...
        if (fsDictionary != nil){
            ...
            ORRaspberryPiMeasurement* aMeasurement  = [ORRaspberryPiMeasurement measurementForBucket:@"Computer" org:org];
...
- (void) decodeBucketList:(NSDictionary*)result
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationOnMainThreadWithName:ORRaspberryPiBucketArrayChanged
                                    object:self
                                  userInfo:nil
                             waitUntilDone:NO];
...
- (void) decodeOrgList:(NSDictionary*)result
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationOnMainThreadWithName:ORRaspberryPiOrgArrayChanged
                                                                        object:self
                                                                      userInfo:nil
                                                                 waitUntilDone:NO];
    [self printOrgTable];
