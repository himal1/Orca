...
- (void) registerNotificationObservers
{
    ...
    [notifyCenter addObserver : self
                     selector : @selector(hostNameChanged:)
                         name : ORRaspberryPiHostNameChanged
                       object : model];
    
    [notifyCenter addObserver : self
                     selector : @selector(authTokenChanged:)
                         name : ORRaspberryPiAuthTokenChanged
                       object : model];
    
    [notifyCenter addObserver : self
                     selector : @selector(orgChanged:)
                         name : ORRaspberryPiOrgChanged
                       object : model];
    
    [notifyCenter addObserver : self
                     selector : @selector(inFluxDBLockChanged:)
                         name : ORRaspberryPiLock
                       object : nil];
    ...
    [notifyCenter addObserver : self
                     selector : @selector(rateChanged:)
                         name : ORRaspberryPiRateChanged
                       object : model];
    
    [notifyCenter addObserver : self
                     selector : @selector(stealthModeChanged:)
                         name : ORRaspberryPiStealthModeChanged
                        object: model];
    
    [notifyCenter addObserver : self
                     selector : @selector(bucketArrayChanged:)
                         name : ORRaspberryPiBucketArrayChanged
                        object: model];
   ...
   [notifyCenter addObserver : self
    ...
    [notifyCenter addObserver : self
                      selector : @selector(errorStringChanged:)
                          name : ORRaspberryPiErrorChanged
                        object : model];
    
    [notifyCenter addObserver : self
                      selector : @selector(connectionStatusChanged:)
                          name : ORRaspberryPiConnectionStatusChanged
                        object : model];
  
    [notifyCenter addObserver : self
                      selector : @selector(maxLineCountChanged:)
                          name : ORRaspberryPiMaxLineCountChanged
                        object : model];
 
    [notifyCenter addObserver : self
                      selector : @selector(measurementTimeOutChanged:)
                          name : ORRaspberryPiMeasurementTimeOutChanged
                        object : model];
}
...
{
    BOOL locked = [gSecurity isLocked:ORRaspberryPiLock];
...
- (void) checkGlobalSecurity
{
    ...
    [gSecurity setLock:ORRaspberryPiLock to:secure];
...
{
    [gSecurity tryToSetLock:ORRaspberryPiLock to:[sender intValue] forWindow:[self window]];
}
...
{
    [model executeDBCmd:[ORRaspberryPiListBuckets listBuckets]];
    [model executeDBCmd:[ORRaspberryPiListOrgs listOrgs]];
}
